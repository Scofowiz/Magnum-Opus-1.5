...
// API URL uses relative path - works with vite proxy in dev and same-origin in production
const API = "";
...
export function Settings(): JSX.Element {
  ...
  const fetchCodexStatus = useCallback(async () => {
    ...
    try {
      const status = await fetchJson(`${API}/api/provider/codex/status`);
  ...
  const fetchModelsWithExistingKey = async (
    ...
    try {
      ...
      if (providerType === "groq") {
        url = `${API}/api/provider/groq/models`;
      } else if (providerType === "google") {
        url = `${API}/api/provider/google/models`;
      } else if (providerType === "ollama") {
        const base = ollamaUrl || "http://localhost:11434";
        url = `${API}/api/provider/ollama/models?baseUrl=${encodeURIComponent(base)}`;
      }
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetchJson(`${API}/api/preferences`),
      fetchJson(`${API}/api/craft-patterns`),
      fetchJson(`${API}/api/lifetime-memory`),
      fetchJson(`${API}/api/provider`),
    ])
  ...
  const fetchModels = async (provider: string, key?: string): Promise<void> => {
    ...
    try {
      ...
      if (provider === "groq") {
        ...
        url = `${API}/api/provider/ollama/models?baseUrl=${encodeURIComponent(ollamaUrl)}`;
      }
  ...
  const clearCraftPatterns = async (): Promise<void> => {
    ...
    try {
      const res = await fetch(`${API}/api/craft-patterns`, {
        method: "DELETE",
      });
  ...
  const clearLifetimeMemory = async (): Promise<void> => {
    ...
    try {
      const res = await fetch(`${API}/api/lifetime-memory`, {
        method: "DELETE",
      });
