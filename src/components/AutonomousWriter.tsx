...
export function AutonomousWriter({
  ...
  const defaultTargetWords = DEFAULT_APP_PREFERENCES.targetWords;
  const [session, setSession] = useState<AutonomousSession | null>(null);
  const [selectedChapterId, setSelectedChapterId] = useState<string | null>(
    chapters[0]?.id || null,
  );
  const [targetWords, setTargetWords] = useState(defaultTargetWords);
  ...
  const [wordsPerChapter, setWordsPerChapter] = useState(defaultTargetWords);
  ...
  const [targetWordsDisplay, setTargetWordsDisplay] =
    useState(defaultTargetWords);
  const [wordsPerChapterDisplay, setWordsPerChapterDisplay] =
    useState(defaultTargetWords);
  ...
  const syncBookModeSelectionOnRefreshRef = useRef(false);
  const MAX_RETRIES = 3;
  ...
  const applySessionSnapshot = useCallback(
    (
      ...
      setTargetWords(nextSession.targetWords);
      setWordsPerChapter(
        nextSession.wordsPerChapter || nextSession.targetWords,
      );
  ...
  useEffect(() => {
    ...
    const loadPreferenceDefaults = async (): Promise<void> => {
      try {
        const preferences = await api.preferences.get();
        if (cancelled || latestSessionRef.current) {
          return;
        }

        const preferredTargetWords =
          preferences.targetWords || defaultTargetWords;
        setTargetWords((current) =>
          current === defaultTargetWords ? preferredTargetWords : current,
        );
        setWordsPerChapter((current) =>
          current === defaultTargetWords ? preferredTargetWords : current,
        );
      } catch (error) {
        console.error("Failed to load autonomous writer preferences:", error);
      }
    };
  ...
  const syncSelectedChapters = useCallback((chapterList: Chapter[]) => {
    const chapterIds = chapterList.map((chapter) => chapter.id);
    setSelectedChapters(chapterIds);
    setSelectedChapterId((previous) =>
      previous && chapterIds.includes(previous)
        ? previous
        : chapterIds[0] || null,
    );
  }, []);
  ...
  const switchToChapterMode = useCallback(() => {
    ...
    setSelectedPlotPoints([]);
    setSelectedThreads([]);
    syncBookModeSelectionOnRefreshRef.current = false;
  }, []);

  const selectAllChapters = useCallback(() => {
    if (mode !== "book") return;
    syncSelectedChapters(chapters);
  }, [chapters, mode, syncSelectedChapters]);
  ...
  useEffect(() => {
    ...
    if (syncBookModeSelectionOnRefreshRef.current) {
      syncBookModeSelectionOnRefreshRef.current = false;
      syncSelectedChapters(chapters);
      return;
    }
    ...
    setSelectedChapters((previous) => {
      const nextSelected = previous.filter((chapterId) =>
        chapterIdSet.has(chapterId),
      );
      return nextSelected.length === previous.length ? previous : nextSelected;
    });
    setSelectedChapterId((previous) =>
      previous && chapterIdSet.has(previous)
        ? previous
        : chapters[0]?.id || null,
    );
  }, [chapters, mode, syncSelectedChapters]);
  ...
  const switchToBookMode = useCallback(async () => {
    ...
    const targetChapterCount = Math.max(
      chapters.length,
      storyBible?.chapterOutlines?.length || 0,
    );
    let chaptersForBookMode = chapters;

    // Prepare chapters whenever the story bible expects more than the project has.
    if (targetChapterCount > chapters.length) {
      try {
        ...
        if (data.chapters && data.chapters.length > chapters.length) {
          chaptersForBookMode = data.chapters;
          syncBookModeSelectionOnRefreshRef.current = true;
    ...
    syncSelectedChapters(chaptersForBookMode);
